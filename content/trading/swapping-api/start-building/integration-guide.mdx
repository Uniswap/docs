...
sidebarTitle: Standard Swap Guide
...
If your integration is built or operated by an AI agent, also consider sending the optional [`X-Agent-Info` attribution header](/docs/trading/swapping-api/start-building/agent-attribution) alongside your request.
...
**Cause**: identified by the `errorCode` field on the 404 body (see the [full reference](/docs/trading/swapping-api/common-errors#404-no-quotes-available))

**Solutions**:
- Handle the 404 by `errorCode`: only `UpstreamTimeoutError` can succeed on an unchanged retry; all other values indicate the request must be modified
- Verify token addresses are correct and on the specified chain (`UnsupportedTokenError`, `NoRouteFoundError`)
- Check chains are available for the selected protocol (`UniswapXNotSupportedOnChainError`; note [limited chain support for UniswapX](/docs/trading/swapping-api/supported-chains))
- Increase trade size for UniswapX minimums (`QuoteAmountTooLowError`)
...
If a quote fails with a 404, use the `errorCode` field to address the specific cause (see the [errorCode reference](/docs/trading/swapping-api/common-errors#404-no-quotes-available)). Only retry when `errorCode` is `UpstreamTimeoutError` (or, cautiously, `ResourceNotFound`) — every other cause is deterministic and will fail again unchanged. Retry with exponential backoff:
...
- **`UNISWAPX_V3`**: Arbitrum, Arc, Avalanche, Base, BNB Smart Chain, Ink, Monad, Robinhood Chain, Tempo, Unichain
- **UniswapX minimum**: a per-chain minimum quotable size (see [Supported Chains](/docs/trading/swapping-api/supported-chains#uniswapx-chain-support))
...
Requests below the minimum threshold return a 404 with `errorCode: QuoteAmountTooLowError`. See [Supported Chains](/docs/trading/swapping-api/supported-chains#uniswapx-chain-support) for details.
