...
If using UniswapX protocols, quote requests must meet a per-chain minimum quotable size. Below that, UniswapX is included only when its quote improves on the AMM route by at least 0.2%.

Requests that meet neither condition return a 404 with `errorCode: QuoteAmountTooLowError`. See [Supported Chains](/docs/trading/swapping-api/supported-chains#uniswapx-chain-support) for details.
