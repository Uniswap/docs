...
faqs:
  ...
  - question: What Permit2 address do I need to whitelist for approvals?
    answer: Approving the Permit2 contract uses the same contract address across supported chains, with a separate address on zkSync. See the Permit2 FAQ for the canonical contract address and audit details.
  ...
  - question: Why am I getting 404 errors from /quote?
    answer: |-
      A 404 from /quote means the request passed schema validation, but the router could not return a quote for it. The errorCode field identifies the cause; the full reference, with what to do for each value, is in the Troubleshooting guide. Common causes: insufficient liquidity for the pair (NoRouteFoundError); a UniswapX-only request below the chain's minimum quotable size (QuoteAmountTooLowError); a token that is not routable (UnsupportedTokenError); or a chain not supported for the operation (UnsupportedChainError). Only UpstreamTimeoutError is safe to retry unchanged.
  ...
  - question: How do I submit a ticket?
    answer: Use the Help Center to reach the team without leaving this page. Note that we are unable to provide technical assistance with troubleshooting your code, deployment, or workflow.
...
<details>
  ...
  Approving the Permit2 contract uses the same contract address across supported chains, with a separate address on zkSync. See the [Permit2 FAQ](/docs/protocols/permit2/overview#faq) for the canonical contract address and audit details.
...
<details>
  <summary>Why am I getting 404 errors from `/quote`?</summary>

  <div style={{ marginInlineStart: '1rem' }}>

  A 404 from `/quote` means the request passed schema validation, but the router could not return a quote for it. The `errorCode` field identifies the cause; the full reference, with what to do for each value, is in [Troubleshooting Common Errors](/docs/trading/swapping-api/common-errors#404-no-quotes-available). Common causes include:
  * There is insufficient liquidity to fill the swap (`errorCode: NoRouteFoundError`)
  ...
  * If making a UniswapX-only request, the amount is below the chain's minimum quotable size (`errorCode: QuoteAmountTooLowError`; see [Supported Chains](/docs/trading/swapping-api/supported-chains#uniswapx-chain-support))
...
<details>
  ...
  UniswapX v2 is supported on Mainnet (chain ID 1), Arbitrum (42161), and Base (8453). UniswapX v3 is supported on Arbitrum (42161), Arc (5042), Avalanche (43114), Base (8453), BNB Smart Chain (56), Ink (57073), Monad (143), Robinhood Chain (4663), Tempo (4217), and Unichain (130).
...
<details>
  ...
  Use the <HelpCenter topic="uniswap-api">Help Center</HelpCenter> to reach the team without leaving this page. Note that we are unable to provide technical assistance with troubleshooting your code, deployment, or workflow.
