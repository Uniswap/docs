...
export const CurrentConfig: SwapExactInSingle = {
    ...
    minHopPriceX36: "0", // Per-hop price floor added in Universal Router 2.1.1. "0" disables it, so amountOutMinimum alone enforces slippage
    hookData: '0x00'
}
```
...
```typescript
const UNIVERSAL_ROUTER_ADDRESS = "0x23617e59A5925b2A4Bf75d73ff6711cD0b29De85" // Universal Router 2.1.2 on Ethereum mainnet. Change the address as per the chain
...
We can get the `UNIVERSAL_ROUTER_ADDRESS` for our chain from the Universal Router 2.1.2 row in [Uniswap Deployments](/docs/protocols/v4/deployments). Universal Router 2.1.2 uses the same calldata as 2.1.1, so the v4 SDK encodes for it with `URVersion.V2_1_1`.
...
// Universal Router 2.1.2 shares its calldata with 2.1.1, so encode the actions for URVersion.V2_1_1
v4Planner.addAction(Actions.SWAP_EXACT_IN_SINGLE, [CurrentConfig], URVersion.V2_1_1);
v4Planner.addAction(Actions.SETTLE_ALL, [CurrentConfig.poolKey.currency0, CurrentConfig.amountIn], URVersion.V2_1_1);
v4Planner.addAction(Actions.TAKE_ALL, [CurrentConfig.poolKey.currency1, CurrentConfig.amountOutMinimum], URVersion.V2_1_1);
...
- `SWAP_EXACT_IN_SINGLE`: This action specifies that we want to perform an exact input swap using a single pool. Its `minHopPriceX36` field is the per-hop price floor added in Universal Router 2.1.1. Setting it to `"0"` disables the per-hop check, so `amountOutMinimum` alone enforces slippage.
