...
export const CurrentConfig: SwapExactIn = {
    ...
    minHopPriceX36: [], // Per-hop price floors added in Universal Router 2.1.1. An empty array disables them, so amountOutMinimum alone enforces slippage
}
```
...
```typescript
const UNIVERSAL_ROUTER_ADDRESS = "0x23617e59A5925b2A4Bf75d73ff6711cD0b29De85" // Universal Router 2.1.2 on Ethereum mainnet
...
The main function for executing multi-hop swaps is very similar to the single-hop guide as well. The only difference is that the first action to the Universal Router is `SWAP_EXACT_IN` instead of `SWAP_EXACT_IN_SINGLE`. As in the single-hop guide, the actions are encoded for `URVersion.V2_1_1`, the calldata format Universal Router 2.1.2 uses, and `minHopPriceX36` is left empty to disable per-hop price floors.
...
v4Planner.addAction(Actions.SETTLE_ALL, [ETH_USDC_POOL_KEY.currency0, CurrentConfig.amountIn], URVersion.V2_1_1);
v4Planner.addAction(Actions.TAKE_ALL, [USDC_USDT_POOL_KEY.currency1, CurrentConfig.amountOutMinimum], URVersion.V2_1_1);
