...
description: Deploy Uniswap v4 hooks by encoding permissions in addresses and using HookMiner patterns.
---

## Hook Flags

As mentioned in [Concept of Hooks](/docs/protocols/v4/concepts/hooks), hook contracts indicate their implemented functions by __encoding their behavior in the address of the contract__. The `PoolManager` uses these permissions to determine which hook functions to call for a given pool. See `PoolManager` deployment addresses [here](/docs/protocols/v4/deployments).
