...
Without this limit, if a subscriber's `notifyUnsubscribe()` consumes too much gas, it could cause the unsubscription transaction to revert, preventing the liquidity provider (LP) from exiting the position.
