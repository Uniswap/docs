...
There is a variable [`unsubscribeGasLimit`](https://github.com/Uniswap/v4-periphery/blob/main/src/base/Notifier.sol#L17) specifically set at deployment of `PositionManager` to prevent gas griefing on `unsubscribe()` - which could result in the subscriber contract not being notified during `unsubscribe()`. 
