...
Moreover, using `extsload` instead of hand-written Solidity view functions helps to keep the contract bytecode size down. This is particularly important for Uniswap v4, as the core contracts are already quite close to Ethereum's contract size limit.
