...
As mentioned above `PoolManager` determines which hook functions to call during the flow of an operation based on the flags encoded directly into the hook's address. 
Each hook function e.g. `afterSwap` is represented by a `0` i.e. false or `1` i.e. true at a specific bit of the hook's address to indicate its implementation.

For local testing, `deployCodeTo` cheatcode in Foundry can be used to deploy hook contract to any address. 

But when deploying hooks to an actual network, address mining will need to be done to find a proper deployment address for the hook to properly represent the functions implemented. 
...
Let's see it in action on Foundry script. We will make use of the example - Points Hook from [Building Your First Hook](./01-your-first-hook.md).
