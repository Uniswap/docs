...
/callout Deltas are not associated to a "token type" therefore developers can mix and match the ERC-20 functions and the ERC-6909 functions
...
Let’s start by creating our external function:
...
5. Context returns to the PoolManager which verifies there are no outstanding deltas, and will relock itself
