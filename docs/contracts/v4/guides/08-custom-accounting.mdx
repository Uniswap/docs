...
:::note
> Using poolManager.take requires an ERC20 balance on the pool manager. For pools where tokens are not custodied by poolmanager, this is a limitation. As a workaround, use either:
>1. poolManager.mint() instead, which is cheaper
