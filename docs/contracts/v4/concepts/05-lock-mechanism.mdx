...
The `Lockers` library stores an array of locker addresses and their corresponding lock callers in transient storage.
...
Functions like `getCurrentHook`, `getHook`, `setCurrentHook`, and `clearCurrentHook` are used for accessing and
modifying these hooks. For instance, `setCurrentHook` sets a hook for the current locker if it hasn't been set
already.
