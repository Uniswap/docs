...
Although the widget will work out-of-the-box with no parameters, we highly recommend integrators provide [JSON-RPC Endpoints](#json-rpc-endpoint) like Infura or Alchemy. Existing dApps can also provide their existing [Web3 provider](#web3-provider) to integrate the widget seamlessly into their dApp.
...
We strongly recommend you provide JSON-RPC endpoint URLs in the widget’s `jsonRpcUrlMap` prop. The widget will use these endpoints to fetch on-chain data and submit transactions for signature. If the user connects a MetaMask wallet, the widget will use the JSON-RPC provided by MetaMask when possible. [(See a list of all chains supported on widget.)](https://github.com/Uniswap/widgets/blob/main/src/constants/chains.ts#L4) 
