...
Otherwise, you can also add your own custom wallet connection flow in the `onConnectWalletClick` callback, which is triggered when the widget's 'Connect your wallet' message is clicked. Disable the widget's built-in wallet connection flow by calling `e?.preventDefault()` in your `onConnectWalletClick` callback. Check out libraries such as [web3-react](https://github.com/Uniswap/web3-react), BlockNative’s [Onboard](https://www.blocknative.com/onboard), or Aragon’s [useWallet()](https://github.com/aragon/use-wallet) to build your own flow.
