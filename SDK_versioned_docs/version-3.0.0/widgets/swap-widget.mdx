...
This guide walks you through the steps to embed the Uniswap Labs swap widget in your web3 decentralized application (dApp). The swap widget lets your users trade tokens directly with the Uniswap Protocol without leaving your dApp.

Here’s a live preview of the swap widget. Please note that wallet connect is not supported here in the documentation. Your app is responsible for providing a method to connect a wallet.
...
The swap widget can help users acquire the right tokens for your dApp or community. Example use cases include:
...
You can drive more engagement in your dApp by embedding the swap experience in your app. You can even customize the default selected tokens, the available token list, and the widget appearance to perfectly fit your use case.
...
By default, the background color of the output module will be set by the `module` attribute. You can optionally enable the color extraction to to flood the output module with the color of the selected output token. In the above example, DAI is the output token so the output module is flooded with DAI yellow.

You can enable color extraction by setting the `tokenColorExtraction` property to `true` in your `Theme` object.
...
The swap widget provides a light and a dark mode variant that you can import as a starting point to create your own themes. If you would like to support both light and dark mode, you can change the value of the `theme` prop to toggle light/dark modes like in the example below.
