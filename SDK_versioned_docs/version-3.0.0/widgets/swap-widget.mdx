...
The swap widget uses a web3 provider to allow users to connect their wallets, fetch balances, and submit transactions through the user’s connected wallet. 

FIXME: onWalletConnect ???

If you don't have a web3 provider yet, simply do not pass anything to the `provider` and `onWalletConnect` props. The widget has built-in wallet connect functionality that supports [WalletConnect](https://walletconnect.com/) and [MetaMask](https://metamask.io/) wallets, making it super simple for you to integrate web3 in your app!
Otherwise, you can also add a custom wallet connect flow to your app by using a library such as [web3-react](https://github.com/NoahZinsmeister/web3-react), BlockNative’s [Onboard](https://www.blocknative.com/onboard), or Aragon’s [useWallet()](https://github.com/aragon/use-wallet).

If you’re building a dApp, you probably have a web3 provider already. Both the [ethers](https://docs.ethers.io/v5/api/providers/provider/) and [web3.js](https://web3js.readthedocs.io/en/v1.5.2/index.html) provider objects are compatible with the widget, as is any [EIP-1193](https://eips.ethereum.org/EIPS/eip-1193) provider. You can pass this directly into the `provider` prop.
...
Next, embed the React component in your application. Pass along the recommended `provider` and `jsonRpcEndpoint` parameters.

```jsx
import { SwapWidget } from '@uniswap/widgets'
import '@uniswap/widgets/fonts.css'

// FIXME: remove provider in this example? To get new users up & spinning
// // Web3 provider as described in the recommendations above
// import { provider } from './your/provider'

// Infura endpoint as described in the recommendations above
